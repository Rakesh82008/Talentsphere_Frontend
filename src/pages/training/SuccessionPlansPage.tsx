...
import type { SuccessionPlanResponse, EmployeeResponse } from '../../types'
...
type SForm = { employeeID: string; successorID: string; status: string }

export default function SuccessionPlansPage() {
  const [plans, setPlans] = useState<SuccessionPlanResponse[]>([])
  const [employees, setEmployees] = useState<EmployeeResponse[]>([])
  const [loading, setLoading] = useState(true)
  const [showCreate, setShowCreate] = useState(false)
  const [editPlan, setEditPlan] = useState<SuccessionPlanResponse | null>(null)
  const [deleteId, setDeleteId] = useState<number | null>(null)
  const [saving, setSaving] = useState(false)
  const [deleting, setDeleting] = useState(false)

  const { register, handleSubmit, reset, formState: { errors } } = useForm<SForm>()
  ...
  const onSubmit = async (data: SForm) => {
    setSaving(true)
    try {
      if (editPlan) {
        await successionsApi.update(editPlan.successionID, { successorID: parseInt(data.successorID), status: data.status as SuccessionPlanResponse['status'] })
        toast.success('Succession plan updated')
        setEditPlan(null)
      } else {
        await successionsApi.create({ employeeID: parseInt(data.employeeID), successorID: parseInt(data.successorID), status: data.status as SuccessionPlanResponse['status'] })
