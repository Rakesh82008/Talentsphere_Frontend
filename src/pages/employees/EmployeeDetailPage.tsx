...
export default function EmployeeDetailPage() {
  ...
  if (!employee) return (
    <div className="text-center py-16">
      <p className="text-gray-700 font-medium mb-2">Employee profile not found.</p>
      <p className="text-sm text-gray-400">Your employee record hasn't been set up yet. Please contact HR or your administrator.</p>
    </div>
  )
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-6">
          <div className="flex flex-col items-center text-center mb-6">
            <div className="h-20 w-20 rounded-full bg-blue-100 flex items-center justify-center mb-4">
              <span className="text-3xl font-bold text-blue-600">{employee.name[0]}</span>
            </div>
        ...
        {/* Performance Reviews */}
        <div className="card p-6">
          <h3 className="font-semibold text-gray-900 mb-4">Performance Reviews</h3>
          {reviews.length === 0 ? (
            ...
            <div className="space-y-3">
              {reviews.slice(0, 5).map((r) => (
                <div key={r.reviewID} className="border border-gray-100 rounded-lg p-3">
                  ...
                  {r.comments && <p className="text-xs text-gray-500 line-clamp-2">{r.comments}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        {/* Career Plans */}
        <div className="card p-6">
          <h3 className="font-semibold text-gray-900 mb-4">Career Plans</h3>
          {careerPlans.length === 0 ? (
            <p className="text-sm text-gray-400">No career plans yet.</p>
          ) : (
            <div className="space-y-3">
              {careerPlans.map((cp) => (
                <div key={cp.planID} className="border border-gray-100 rounded-lg p-3">
                  <div className="flex justify-between items-center mb-1">
                    <p className="text-sm font-medium text-gray-900">{cp.title}</p>
                    <StatusBadge status={cp.status} />
                  </div>
                  {cp.description && <p className="text-xs text-gray-500 line-clamp-2">{cp.description}</p>}
                </div>
              ))}
            </div>
          )}
