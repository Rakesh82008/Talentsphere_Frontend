...
export default function InterviewsPage() {
  ...
  const { register: regStatus, handleSubmit: handleStatusSubmit, reset: resetStatus } = useForm()
  ...
  const load = async () => {
    ...
    try {
      const [iv, apps, u] = await Promise.allSettled([
        ...
        // Only load applications that are eligible for interview scheduling
        applicationsApi.getAll({ page: 1, pageSize: 100 }),
        // Only load HR, Manager, Recruiter users — not all users
        usersApi.getInterviewers(),
      ])
        ...
        <form className="space-y-4">
          <Select label="Application" required placeholder="Select application" error={errors.applicationID?.message}
            options={applications
              // Only applications in a state eligible for interview scheduling
              .filter((a) => ['Submitted', 'Reviewed'].includes(a.status))
              .map((a) => ({ value: a.applicationID, label: `${a.candidateName ?? 'Candidate'} — ${a.jobTitle ?? 'Job #' + a.jobID} (${a.status})` }))}
