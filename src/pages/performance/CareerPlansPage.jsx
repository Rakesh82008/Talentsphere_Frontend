...
export default function CareerPlansPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (editPlan) {
        await careerPlansApi.update(editPlan.planID, { title: data.title, description: data.description, status: data.status })
        toast.success('Career plan updated')
        setEditPlan(null)
      } else {
        await careerPlansApi.create({ employeeID: parseInt(data.employeeID), title: data.title, description: data.description, status: data.status })
